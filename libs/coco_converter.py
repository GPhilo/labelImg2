...
def export_annotations(img_path, anno_path, out_path):
  files = collect_files(img_path, anno_path)
  image_infos = collect_annotations(files, nproc=6)
  convert_annotations(
    list(filter(None, image_infos)),
    out_path)

class ExporterThread(QThread):
  def __init__(self, img_path, anno_path, out_path, parent=None):
  ...
  def run(self):
    export_annotations(self.img_path, self.anno_path, self.out_path)